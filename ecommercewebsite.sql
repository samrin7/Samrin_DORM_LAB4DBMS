...
select cus_gender, count(*) as Gender_Count 
from customer 
where cus_id in (
	select cus_id 
    from `order` 
...
select c.cat_id,cat_name , pro_name , min(sp.supp_price) as Price 
from category c 
...
/* Ques. 9 _> Create a stored procedure to display supplier id, name, rating and Type_of_Service. For Type_of_Service,
...
DELIMITER //
CREATE PROCEDURE DisplayServiceType()
BEGIN
